/* 
	Universal Virtual Device Handler 
	...
	Home Page: https://community.smartthings.com/t/release-universal-virtual-device-type-and-translator/47836

		OLD definition (name: "uDTH", namespace: "MikeMaxwell", author: "mike maxwell") 
*/

metadata {
	definition(name: "Universal Virtual Switch", namespace: "Virtual", author: "Murali Kesavan",
    ...
    preferences {
        ...
        input( 
        	name			: "contactOn"
           	...
           	,description	: contactOn ?: "Not Used, Tab to enable..."
        )        
		...
		input( 
            ...
            ,type			: "enum"
            ,options		: buildOptions(d,s1,s2)
            ,description	: motionOn ?: "Not Used, Tab to enable..."            
        )
		...
		input( 
            ...
            ,type			: "enum"
            ,options		: buildOptions(d,s1,s2)
            ,description	: smokeOn ?: "Not Used, Tab to enable..."            
        )
		...
		input( 
            ...
            ,type			: "enum"
            ,options		: buildOptions(d,s1,s2)
            ,description	: waterOn ?: "Not Used, Tab to enable..."            
        )
		...
		input( 
            ...
            ,type			: "enum"
          	,options		: buildOptions(d,s1,s2)
            ,description	: presenceOn ?: "Not Used, Tab to enable..." 
        )
        ...
        input( 
            ...
            ,type			: "enum"
            ,options		: buildOptions(d,s1,s2)
            ,description	: doorOn ?: "Not Used, Tab to enable..." 
        )
        d = "Acceleration"
        s1 = "active"
        s2 = "inactive"
		input( 
            ...
            ,type			: "enum"
          	,options		: buildOptions(d,s1,s2)
            ,description	: accelOn ?: "Not Used, Tab to enable..."
        )
        d = "Illuminance"
        s1 = "0 Lux"
        s2 = "50 Lux"
		input( 
            ...
            ,type			: "enum"
          	,options		: buildOptions(d,s1,s2)
            ,description	: luxOn ?: "Not Used, Tab to enable..."
        )        
    }
	...
	tiles (scale:1) {
    	multiAttributeTile(name:"switch", type: "generic", width: 6, height: 2, canChangeIcon: true){
			tileAttribute ("device.uVDH", key: "PRIMARY_CONTROL") {
...
def syncDevices(cmd){
	if (cmd == null) cmd = device.currentValue("uVDH") == "on" ? "1" : "0"
    //log.debug "cmd: ${cmd}"
	if (contactOn in ["0","1"]){
...
def localOn() {
	if (device.currentValue("uVDH") != "on"){
    	log.info "on request: OK"
		sendEvent(name: "uVDH", value: "on" ,displayed: false)
    	syncDevices("1")
...
def localOff() {
	if (device.currentValue("uVDH") != "off"){
    	log.info "off request: OK"
		sendEvent(name: "uVDH", value: "off" ,displayed: false)
    	syncDevices("0")
    } else {
